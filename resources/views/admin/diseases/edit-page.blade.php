      ...
      <div class="col-12 col-md-6 order-md-2 order-first">
        <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{url('/admin/diseases')}}">Diseases</a></li>
              ...
              <div class="row">
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="Enter Disease Name" value="{{ $rowData->name ?? '' }}" name="name" id="name">
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="dept_id">Department</label>
                    <select class="form-select" name="dept_id" id="dept_id" required>
                      <option value="">Select Department</option>
                      @foreach($departments as $dept)
                      <option value="{{ $dept->id }}" {{ (isset($rowData->dept_id) && $rowData->dept_id == $dept->id) ? 'selected' : '' }}>{{ $dept->name }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
...
<script>
    let saveDataURL = "{{url('/admin/edit-disease/'.$row_id)}}";
    let returnURL = "{{url('/admin/diseases')}}";
</script>
<script src="{{ asset('public/admin/js/pages/diseases/edit-page.js') }}"></script>
@endsection
