...
<div class="page-heading">
  <div class="page-title">
    <div class="row">
      <div class="col-12 col-md-6 order-md-1 order-last">
        <h3>OPD Reports</h3>
        <p class="text-subtitle text-muted">Filter by date range and department to view OPD reports.</p>
      </div>
  ...
  <section class="section">
    ...
    <div class="card">
      <div class="card-body">
        <form id="opdReportsForm" method="post" action="{{ url('/admin/opd-reports') }}">
          @csrf
            ...
            <div class="col-12 col-md-3">
              <div class="form-group mb-3">
                ...
                <select class="form-select" id="dept_id" name="dept_id">
                  ...
                  <option value="{{ $dept->id }}" {{ old('dept_id', $filterDeptId ?? '') == $dept->id ? 'selected' : '' }}>{{ $dept->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
    ...
    @if(isset($datewiseData) && count($datewiseData) > 0)
      <div class="mb-2">
        <a href="{{ url('/admin/opd-reports-export') }}?from_date={{ urlencode($fromDate ?? '') }}&to_date={{ urlencode($toDate ?? '') }}&dept_id={{ $filterDeptId ?? '' }}" class="btn btn-success btn-sm">
            ...
            <table class="table table-bordered mb-0">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ $opd->patient_age ?? '—' }} / {{ $opd->gender ?? '—' }}</td>
                  <td>{{ $opd->address ?? '—' }}</td>
                  <td>{{ $opd->department ? $opd->department->name : '—' }}</td>
                  <td>{{ $opd->disease ? $opd->disease->name : '—' }}</td>
                  <td>{{ $opd->register_type ?? '—' }}</td>
                </tr>
                @endforeach
              </tbody>
            </table>
